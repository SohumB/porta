...
class Admin::Api::BuyersApplicationsTest < ActionDispatch::IntegrationTest
  ...
  test 'index states' do
    ...
    @buyer.reload
    assert @buyer.bought_cinstances.find_by(plan_id: pending_plan.id).pending?
    ...
    assert @buyer.bought_cinstances.find_by(plan_id: suspend_plan.id).suspended?

    %i[pending live suspended].each do |state|
      get admin_api_account_applications_path(@buyer, format: :xml), params: { state: state, provider_key: @provider.api_key }

      assert_response :success

      assert_applications(response.body, { state: state, user_account_id: @buyer.id })
    end
  end
  ...
  test 'show returns defined fields' do
    application = @buyer.bought_cinstances.last
    application.update(name: "tomatoes < > &", description: "rotten < > &")
  ...
  test 'show returns defined fields for json' do
    application = @buyer.bought_cinstances.last
    application.update(name: "CoinBase", description: "Mining bitcoins in your screesaver like a boss")
  ...
  test 'find by user_key on backend v1' do
    ...
    get find_admin_api_account_applications_path(@buyer.id, format: :xml), params: { user_key: @application.user_key, provider_key: @provider.api_key }
  ...
  test 'find by app_id on backend v2' do
    ...
    get find_admin_api_account_applications_path(@buyer.id, format: :xml), params: { app_id: @application.application_id, provider_key: @provider.api_key }
  ...
  test 'create with extra fields' do
    ...
    post admin_api_account_applications_path(@buyer, format: :xml), params: { plan_id: @hidden_app_plan.id, name: "extra app", description: "extra app", some_extra_field: 'extra value', some_other_extra_field: 'other extra value', provider_key: @provider.api_key }
    ...
    assert_response :success
    assert_application(response.body, name: "extra app", extra_fields: extra_fields)
  ...
  test 'update with long user_key' do
    ...
    put admin_api_account_application_path(@buyer, id: app.id, format: :xml), params: { user_key: key, provider_key: @provider.api_key }
  ...
  test 'customize_plan' do
    ...
    assert_not_equal plan.id.to_s, xml.xpath('.//plan/id').children.first.to_s
  ...
  test 'change plan to existing plan should not trigger 500 error' do
    put change_plan_admin_api_account_application_path(@buyer, @buyer.application_contracts.first, provider_key: @provider.api_key, plan_id: @published_app_plan.id, format: :xml)
    assert_response :success

    put change_plan_admin_api_account_application_path(@buyer, @buyer.application_contracts.first, provider_key: @provider.api_key, plan_id: @published_app_plan.id, format: :json)
    assert_response :success
  end
  ...
  test 'change application plan' do
    ...
    assert_equal @buyer.application_contracts.first.plan, @published_app_plan

    #TODO: dry plan xml assertion into a helper
    #testing xml response
    xml = Nokogiri::XML::Document.parse(response.body)
    assert_equal xml.xpath('.//plan/id').children.first.to_s, @published_app_plan.id.to_s
  end

  test 'change application plan to a non-published one is permitted' do
    assert_equal @buyer.application_contracts.first.plan, @app_plan
    ...
    #testing xml response
    xml = Nokogiri::XML::Document.parse(response.body)
    assert_equal xml.xpath('.//plan/id').children.first.to_s, @hidden_app_plan.id.to_s
  end
  ...
  test 'accept' do
    @published_app_plan.update(approval_required: true)
